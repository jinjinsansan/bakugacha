-- 036: play_gacha に effective_result を返却追加
-- max_winners上限でlossに変更された場合もAPIが正しいアニメーションを表示できるようにする
...
BEGIN
  ...
  -- Admin fast-path: verify product exists, return success without side effects
  IF p_is_admin THEN
    IF p_product_id IS NOT NULL THEN
      PERFORM 1 FROM gacha_products WHERE id = p_product_id;
      IF NOT FOUND THEN
        RETURN QUERY SELECT FALSE, 'PRODUCT_NOT_FOUND'::TEXT,
                             NULL::UUID, 0, NULL::TEXT, NULL::UUID, NULL::INT, p_result::TEXT;
        RETURN;
      END IF;
    END IF;
    ...
    RETURN QUERY SELECT TRUE, NULL::TEXT,
                         NULL::UUID, v_coins, NULL::TEXT, NULL::UUID, NULL::INT, p_result::TEXT;
    RETURN;
  END IF;

  -- 1. lock user row and check coins
  IF p_user_id IS NOT NULL THEN
    IF p_price > 0 THEN
      ...
      IF v_coins IS NULL THEN
        RETURN QUERY SELECT FALSE, 'USER_NOT_FOUND'::TEXT,
                             NULL::UUID, 0, NULL::TEXT, NULL::UUID, NULL::INT, NULL::TEXT;
        RETURN;
      END IF;

      IF v_coins < p_price THEN
        RETURN QUERY SELECT FALSE, 'INSUFFICIENT_COINS'::TEXT,
                             NULL::UUID, v_coins, NULL::TEXT, NULL::UUID, NULL::INT, NULL::TEXT;
        RETURN;
      END IF;
    ELSE
      SELECT coins INTO v_coins FROM app_users WHERE id = p_user_id;
  ...
  -- 2. atomic stock decrement + fetch max_winners
  IF p_product_id IS NOT NULL THEN
    ...
    -- 2b. max_winners check: force loss if cap already reached
  ...
  IF p_user_id IS NOT NULL AND p_price > 0 THEN
    UPDATE app_users
    ...
    VALUES (p_user_id, 'gacha', -p_price, v_coins, 'gacha: ' || p_prize_name);
  END IF;

  -- 4. insert gacha_results
  ...
  -- 5. card issuance (only if card_info is present)
      ...
      IF v_max_issuance > 0 THEN
        ...
        IF v_current_count >= v_max_issuance THEN
          RETURN QUERY SELECT TRUE, 'CARD_MAX_REACHED'::TEXT,
                               v_result_id, v_coins, NULL::TEXT, NULL::UUID, NULL::INT, v_effective_result::TEXT;
          RETURN;
        END IF;
      END IF;
  ...
  -- 6. prize_claims (cd2/ecard only, on effective win)
  ...
  RETURN QUERY SELECT TRUE, NULL::TEXT,
                       v_result_id, v_coins, v_serial, v_new_card_id, v_next_seq, v_effective_result::TEXT;
END;
$$ LANGUAGE plpgsql;
