    ...
    >
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          className={
            claimed
              ? 'px-4 py-2 rounded-xl text-xs font-black bg-white/5 text-gray-500 cursor-not-allowed whitespace-nowrap'
              : 'btn-gold px-5 py-2 rounded-xl text-xs font-black tracking-wider whitespace-nowrap disabled:opacity-60'
          }
        >
          {claimed ? '受け取り済み' : isPending ? '処理中...' : '🎁 受け取る'}
        </button>
      </div>
    </div>
  );
